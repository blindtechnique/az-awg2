...
ask_yn() {  # ask_yn "<вопрос>" <default: y|n> → 0 если да
    # Непонятный ответ ПЕРЕСПРАШИВАЕМ, а не трактуем как «нет». Раньше любой
    # неузнанный ввод молча означал отказ, и это било по живым установкам:
    # достаточно набрать «н», стереть и написать «y», чтобы бот не поставился.
    # Причина — кириллическая буква в UTF-8 занимает два байта, а backspace на
    # tty без флага IUTF8 (обычное дело на свежем сервере без локали) стирает
    # ОДИН байт. В буфере остаётся хвост \xd0, поверх него «y», и ни один case
    # не совпадает. Теперь такой ввод виден пользователю, а не пропадает молча.
    local q="$1" def="${2:-n}" a src=/dev/stdin tries=0
    [ -r /dev/tty ] && src=/dev/tty
    while :; do
        a=""
        read -rp "$q" a < "$src" || a="$def"      # EOF или нет ввода — по умолчанию
        ...
        # Ограничение обязательно: без tty ввода не появится никогда, и цикл
        # крутился бы вечно на неинтерактивной установке.
        tries=$((tries + 1))
        if [ "$tries" -ge 5 ]; then

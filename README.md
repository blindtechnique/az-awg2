...
- **Слой AmneziaWG 3.0** — в ветке [`beta`](#ветка-beta-amneziawg-30): header protection, content padding и рандомизация таймингов, вторым слоем рядом с 2.0.
...
## Ветка beta: AmneziaWG 3.0

В `beta` обкатывается **второй слой — AmneziaWG 3.0**. Он не заменяет 2.0, а поднимается рядом: свои интерфейсы, свои подсети, свой UDP-порт и свой профиль обфускации. Клиентам со свежими приложениями выдаёшь 3.0, всем остальным — 2.0, и ничего не ломается.

Что 3.0 добавляет к транспорту поверх обфускации 2.0:

- **Header protection** — заголовок пакета шифруется отдельным ключом, поэтому WireGuard-подобная структура не видна даже в первом байте.
- **Content padding** — длина полезной нагрузки размывается, и характерные размеры пакетов перестают выдавать протокол.
- **Рандомизация таймингов** — интервалы rekey, keepalive и повторов хендшейка перестают быть постоянными, по которым туннель узнаётся статистически.

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/blindtechnique/az-awg2/beta/install.sh)
```

Клиенты слоя 2.0 переход переживают: профиль обфускации не перевыпускается, конфиги остаются байт в байт прежними, переимпортировать ничего не нужно. По окончании `regen-all` сам скажет — `Конфиги клиентов: 87 без изменений — переимпорт не нужен`. Если слой 3.0 не нужен, а хочется только остального, добавь `--awg 2`.

> [!IMPORTANT]
> Слой 3.0 работает на **userspace-датапасе** `amneziawg-go` (пин `v3.0.20260805`), а не на kernel-модуле. Модуль параметры 3.0 уже знает, но переезд отложен: на нём открыты [kmod #215](https://github.com/amnezia-vpn/amneziawg-linux-kernel-module/issues/215) и [amnezia-client #3043](https://github.com/amnezia-vpn/amnezia-client/issues/3043) — обе «хендшейк проходит, трафика нет», обе без ответа мейнтейнеров. Версия 3.1 по тем же причинам **не используется**. Слою 2.0 это не касается: он как был на kernel-модуле, так и остаётся.

> [!NOTE]
> Клиенту нужно приложение Amnezia не старее **5.0.0.2** — более ранние сборки параметры 3.0 просто игнорируют и не сходятся с сервером. Начиная с 5.0.1.5 приложение подписывает такой профиль как «AmneziaWG (version 3.1)»: оно считает версию само и знает ровно одну тройку. Сервер при этом отдаёт 3.0, на работу это не влияет.

Помимо самого слоя `beta` приносит:

- **Раздельная обфускация и порты для каждого слоя** — пресет 2.0 и пресет 3.0 задаются независимо, потому что «интенсивность» значит для них разное: на `router` и `low` слой 3.0 объявлен без header protection и вырождается в 2.0. Установщик про это предупреждает.
- **Диагностика по слоям** — `awg-doctor` различает 2.0 и 3.0, спрашивает у демона через UAPI, действительно ли параметры 3.0 применены, и не считает их отсутствие поломкой там, где пресет их и не обещал. Плюс `--deep`: поднимает временного клиента в network namespace и гоняет через туннель настоящий трафик.
- **Бот стал осторожнее** — постраничный список клиентов, смена пресета обфускации с экраном последствий (сколько человек потеряют доступ) и подтверждением, долгие операции ушли в фоновые задания с живым логом.
- **Приёмка и CI** — наборы лежат в `tests/`, запускаются одной командой `bash tests/run.sh` и гоняются на каждый push.

Статус — обкатка: ветка меняется чаще, чем `main`. Вернуться обратно можно в любой момент, слой 3.0 при этом останется на диске и продолжит работать:

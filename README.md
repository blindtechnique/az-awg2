...
Последнее обновление — [**3.0.0-rc.8**](CHANGELOG.md#300-rc8--2026-09-08): исправлены применение настроек AWG2 и ложные ошибки глубокой диагностики. Для работающих подключений переимпорт конфигов не требуется.
...
### Применение AWG2 из CLI и сторонних панелей

Обфускатор читает имена интерфейсов из `services.env`, в том числе при прямом
вызове из панели без переменных окружения. В `parallel` изменяются только
`antizapret-awg` / `vpn-awg` (либо настроенные имена), не ванильные интерфейсы.
Оба файла проверяются **до** изменения профиля. Отсутствующий файл, цель вне
каталога AWG, симлинк, совпадающие цели или попытка попасть в AWG3 дают отказ.

После перезапуска проверяются **env → серверный .conf → живой AWG2**:
обфускация, ключи/пиры/PSK/AllowedIPs и порт. Проверку выполняет
`awg2-verify-profile.py`; он устанавливается и обновляется вместе со слоем,
не печатает ключи и используется также в `awg-doctor`.

Ошибка применения возвращает код `3` даже для изначально выключенного AWG2.
Отсутствие юнита также не означает успешного применения. Инсталлятор может
продолжить после этого промежуточного состояния, но обязан запустить и
проверить AWG2 до выдачи клиентских конфигов. Панели должны прерывать операцию
при ненулевом коде и вызывать `awg-client regen-all` **только после успеха**.
Для генерации нового профиля следует явно передавать `--regenerate --apply`.

`--reapply` повторно применяет сохранённый профиль **без его перевыпуска**.
Обычное `--update` доставляет новый код, но не применяет профиль и не меняет
клиентов. Полная транзакция с откатом, общая блокировка операций и проверка
профиля, уже импортированного на устройстве, в это исправление не входят.
Пресеты и шаблоны не менялись: `low + web` — существующая комбинация.
...
| через Wi-Fi работает, через мобильную сеть не подключается | Возможна выборочная фильтрация UDP по порту и внешней подсети сервера. Проверь тот же профиль на другом свободном UDP-порту; [подробнее ниже](#мобильные-сети-udp-порты-и-подсеть-vps). Не меняй одновременно обфускацию и DNS |
...
### Мобильные сети: UDP-порты и подсеть VPS

Если один и тот же профиль работает через домашний интернет, но не через мобильную сеть, причина может быть не в обфускации. Фильтрация бывает выборочной: доступность зависит от оператора, внешнего IP/подсети VPS или сети хостера (AS), порта и времени проверки.

В [опубликованном 26 февраля 2026 года наблюдении](https://habr.com/ru/articles/1003852/) описана временная блокировка UDP-портов **выше 1000** для определённых внешних сетей после срабатывания фильтра. Это не постоянное правило для всего интернета: автор позднее отметил изменение механизма блокировки. Поэтому работа высокого порта на одном VPS **не исключает** его блокировку на другом, даже у того же оператора. И наоборот, нельзя обещать, что любой порт ниже 1000 обязательно заработает.

Здесь **подсеть VPS — это сеть его публичного IP**, а не внутренние адреса VPN (`10.28.x.x` / `10.29.x.x`) или fake-IP AntiZapret (`10.30.0.0/15` / `198.18.0.0/15`). Замена внутренних диапазонов не меняет внешний адрес, который видит оператор.

**Как проверить:**

1. Сохрани рабочий профиль. Для отдельного тестового подключения меняй только внешний UDP-порт, оставляя тот же сервер, ключи, обфускацию, DNS и маршруты.
2. Если высокий порт недоступен, в первую очередь проверь свободный низкий порт, например **UDP 443**. Проверь, что вход разрешён на VPS и в сетевом экране хостера, а `Endpoint` клиента указывает на этот вход.
3. Проверяй из проблемной сети и установление соединения (handshake), и открытие сайтов. Надписи «подключено» или успешной локальной диагностики на сервере недостаточно. Повтори подключение: один удачный сеанс не доказывает постоянную доступность.
4. Если решишь сменить порт постоянно, согласуй серверные настройки, внешний вход и клиентские конфиги. Ранее выданные профили сами не обновятся: `regen-all` не меняет `Endpoint`. Не переноси порты уже работающих серверов только потому, что другой VPS потребовал низкий порт.

**Проверь занятость порта.** На одном IP разные AWG-интерфейсы не могут просто занять один и тот же UDP-порт: для AntiZapret и FullVPN нужны отдельные UDP-порты. TCP 443 для HTTPS или VLESS не занимает UDP 443, но Hysteria2 или другой QUIC-сервис на UDP 443 уже будет конфликтовать. Выбор порта — отдельная настройка подключения, а не пресет обфускации; установщик не переносит все порты ниже 1000 автоматически.

**Ошибки DNS проверяй отдельно.** Если соединение установлено, но сайты не открываются по имени, проверь DNS и актуальность маршрутов (`AllowedIPs`) в импортированном профиле. Переход на низкий порт не обновляет эти настройки. Список маршрутов зависит от настроек AntiZapret: универсального «правильного количества» нет, обрезать его до фиксированного числа не нужно.

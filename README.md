...
> [!WARNING]
> `--reconfigure` — отдельная операция: он **намеренно** выпускает новый профиль обфускации, и тогда все розданные конфиги 2.0 перестают подключаться, пока их не переимпортируют. Для перехода между ветками он не нужен.

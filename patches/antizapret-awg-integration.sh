...
switch_services() {
    ...
    # Без глушилки: шаг, который умеет тихо не сработать, и вызов, который это
    # проглатывает, — так пропажа DNS-view и прожила незамеченной.
    "$DEST/awg-knot-view.sh" || err "DNS: view для подсетей слоя не добавлены (см. вывод выше)"
}

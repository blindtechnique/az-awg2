...
# Срез до VPN3_SUBNET, а не до VPN_SUBNET: раньше он обрывался двумя строками
# раньше слоя 3.0 — ровно там, где закрепления и не было.
pin="$(sed -n '/^    local pin_az="" pin_vpn="" pin_az3="" pin_vpn3=""$/,/^    VPN3_SUBNET=/p' \
       patches/antizapret-awg-integration.sh)"
if [ -z "$pin" ]; then
    echo "  ✘ подсети не закрепляются — переезд ванили уведёт сервер от клиентов"; fail=1
else
    p() {  # p <есть services.env: 1|0> <какую переменную вернуть>
        local d; d="$(mktemp -d)"
        [ "$1" = 1 ] && printf 'AZ_SUBNET=10.29.9\nVPN_SUBNET=10.28.9\nAZ3_SUBNET=10.29.10\nVPN3_SUBNET=10.28.10\n' \
            > "$d/services.env"
        # shellcheck disable=SC2034
        ( SERVICES="$d/services.env"; az_base=10.29.99; vpn_base=10.28.99
          eval "$pin"; eval "echo \"\$$2\"" )
        rm -rf "$d"
    }
    [ "$(p 1 AZ_SUBNET)" = "10.29.9" ] \
        && echo "  ✔ закреплённая подсеть 2.0 переживает переезд ванили" \
        || { echo "  ✘ подсеть 2.0 пересчитана заново — у клиентов остался старый Address"; fail=1; }
    [ "$(p 0 AZ_SUBNET)" = "10.29.100" ] \
        && echo "  ✔ на первой установке 2.0 считается от ванили" \
        || { echo "  ✘ на первой установке подсеть 2.0 не вычисляется: $(p 0 AZ_SUBNET)"; fail=1; }
    # Слой 3.0: обоснование закрепления было написано в комментарии выше и не
    # применено к нему. Переезд ванили уводил серверные интерфейсы 3.0, а у
    # клиентов оставался прежний Address — regen-all адреса не трогает.
    [ "$(p 1 AZ3_SUBNET)" = "10.29.10" ] \
        && echo "  ✔ закреплённая подсеть 3.0 тоже переживает переезд" \
        || { echo "  ✘ подсеть 3.0 пересчитана заново: $(p 1 AZ3_SUBNET)"; fail=1; }
    [ "$(p 1 VPN3_SUBNET)" = "10.28.10" ] \
        && echo "  ✔ и вторая подсеть 3.0" \
        || { echo "  ✘ вторая подсеть 3.0 пересчитана: $(p 1 VPN3_SUBNET)"; fail=1; }
    [ "$(p 0 AZ3_SUBNET)" = "10.29.101" ] \
        && echo "  ✔ на первой установке 3.0 считается от ванили" \
        || { echo "  ✘ на первой установке подсеть 3.0 не вычисляется: $(p 0 AZ3_SUBNET)"; fail=1; }
fi

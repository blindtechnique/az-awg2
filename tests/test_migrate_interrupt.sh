...
if ! unshare -Urm --map-root-user true 2>/dev/null; then
    ...
    # В CI зелёный прогон одинаково выглядит и когда набор отработал, и когда
    # он себя пропустил. Разница существенная: во втором случае настоящая
    # миграция не проверялась вовсе. Аннотация публична и видна без входа в
    # аккаунт, в отличие от логов.
    [ -n "${GITHUB_ACTIONS:-}" ] && printf '::notice title=%s::%s\n' \
        "test_migrate_interrupt" \
        "пропущен: unshare -Urm недоступен, настоящая миграция не проверялась"
    exit 0
fi

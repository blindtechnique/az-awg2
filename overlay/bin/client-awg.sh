...
server_host() {
    ...
    if [ -z "$h" ]; then
        # `|| true` внутри подстановки обязателен: правый операнд || — последняя
        # команда списка, поэтому set -e срабатывает на ней в полную силу, и
        # греп без совпадения обрывал скрипт, не доходя до запасного варианта.
        ...
        # никогда: 192.168.1.10 у домашнего сервера в своей же локалке законен.

...
# userspace только если это НЕ удалось. То есть на машине с загруженным
# модулем он МОЛЧА поднял бы 2.0-датапас вместо 3.0. Поэтому мы явно и всегда
# запускаем userspace-демон amneziawg-go.
#
# Параметры 3.0 модуль, к слову, уже знает — апстрим влил их в master
# 30.07.2026 (PR #192). Но переезд отложен: на модуле открыты issues #215 и
# amnezia-client #3043, обе «хендшейк проходит, трафика нет», и вторая прямо
# сообщает, что ядро 3.1 и userspace 3.1 ведут себя одинаково.
#

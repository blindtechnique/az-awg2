...
#   * systemd-юниты и drop-in'ы в /etc/systemd/system/.
#
# Что сбрасывает ручной `setup.sh` (rm -rf /root/antizapret) и что чиним здесь:
#   1. отсутствующий хук в custom-up.sh → дописываем;
#   2. слетевшие симлинки /usr/local/bin → пересоздаём;
#   3. не поднятые awg-quick@ интерфейсы → поднимаем;
#   4. затёртый view в kresd.conf для наших подсетей → возвращаем (awg-knot-view.sh).
#
# В режиме parallel ваниль НЕ трогается вообще: wg-quick живёт своей жизнью,
# up.sh не патчится. Legacy-ветки (replace/keep) сохранены для НЕмигрированных
# установок прошлых версий — до запуска install.sh --migrate.
#
# Идемпотентно и НИКОГДА не фатально (always exit 0) — вызывается из drop-in
# antizapret.service.d (каждый старт antizapret), из awg-reintegrate.service
# (загрузка) и из custom-up.sh.
export LC_ALL=C
...
MODE="${MODE:-replace}"
# дефолты имён интерфейсов зависят от режима (в старом replace они совпадали с ванилью)
if [ "$MODE" = parallel ] || [ "$MODE" = keep ]; then
    AZ_IFACE="${AZ_IFACE:-antizapret-awg}"; VPN_IFACE="${VPN_IFACE:-vpn-awg}"
else
    AZ_IFACE="${AZ_IFACE:-antizapret}"; VPN_IFACE="${VPN_IFACE:-vpn}"
fi

# 1. симлинки CLI (пересоздаём всегда — дёшево и идемпотентно)
...
# 2. hook в custom-up.sh (upstream сбрасывает его при setup.sh)
...
# 3. наши awg-quick@ должны быть подняты И быть типа amneziawg. Проверяем ТИП
#    интерфейса напрямую (ip -d link), а не systemd-состояние: интерфейс с тем же
#    именем мог создать кто-то другой (тип wireguard), и тогда awg show пустой.
...
# ═══ LEGACY-ветки для немигрированных установок (до install.sh --migrate) ═════

# replace: ванильный wg-quick@ мог быть заново включён setup.sh — душим
...
if [ "$MODE" = replace ]; then
    for s in wg-quick@antizapret wg-quick@vpn; do
        if systemctl is-enabled "$s" >/dev/null 2>&1 || systemctl is-active "$s" >/dev/null 2>&1; then
            systemctl disable --now "$s" 2>/dev/null && log "legacy replace: отключён ванильный $s"
        fi
    done
fi

# keep: setup.sh возвращает в up.sh редирект 52xxx→51xxx, который снова перекрывает
# наш AmneziaWG (52443/52080). up.sh применяет правило ДО вызова custom-up.sh,
# поэтому чиним и файл (на будущее), и живой iptables.
if [ "$MODE" = keep ]; then
    ...
    if [ -f "$up" ] && grep -qE 'dport 5(2443|2080) -j REDIRECT' "$up"; then
        ...
        log "legacy keep: up.sh — убран редирект 52xxx→51xxx"
    fi
    ...
    log "legacy keep: живой редирект 52xxx снят, порты 52443/52080 открыты"
fi

...
PYBK
    ...
    lock_drop
    # umask ДО упаковки, а не chmod после. В архиве приватный ключ CA OpenVPN,
    # ключи серверных интерфейсов и приватные ключи ВСЕХ клиентов; пока идёт
    # tar — на сотне клиентов это десятки секунд — файл существовал с правами
    # по умолчанию, и открывший его в этом окне дочитывал до конца уже после
    # chmod. --encrypt не спасал: он шифрует уже созданный открытый файл.
    umask 077
    tar -czf "$out" -C "$stage" .
    rm -rf "$stage"
    chmod 600 "$out"
